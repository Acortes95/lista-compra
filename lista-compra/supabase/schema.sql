...
create policy "task_list_items_select_member" on public.task_list_items for select
  using (exists (select 1 from public.tasks t where t.id = task_list_items.task_id and public.is_member_of_group(t.group_id)));
create policy "task_list_items_insert_member" on public.task_list_items for insert
  with check (exists (select 1 from public.tasks t where t.id = task_list_items.task_id and t.group_id = task_list_items.group_id and public.is_member_of_group(t.group_id)));
create policy "task_list_items_update_member" on public.task_list_items for update
  using (exists (select 1 from public.tasks t where t.id = task_list_items.task_id and t.group_id = task_list_items.group_id and public.is_member_of_group(t.group_id)));
